...
public class KangarooRace extends GameTime {
	...
	public void start() {
		...
		for (int i = 0; i < gamerManager.getAliveGamers().size(); i++) {
			...
			if (!end.contains(gamer)) {
				...
				manager.getUtils().teleportPlayer(gamer.getPlayer(), "starpvp.party.kangaroo.spawns." + (i+1));
			}
		}
		...
		new BukkitRunnable() {
			public void run() {
				...
				for (Gamer gamers : gamerManager.getGamers()) {
					if (gamers.isAlive() && !end.contains(gamers) && gamers.getPlayer().getLocation().add(0, -1, 0).getBlock().getType().equals(Material.DIAMOND_BLOCK)) {
						...
						if (end.size() > 0) {
							chestplate = new ItemBuilder().setColor(Material.LEATHER_CHESTPLATE, Color.YELLOW, "§aPeitoral");
							Messages.sendBroadcast("§aO jogador §f" + gamers.getName() + "§a terminou a corrida §e(" + playerTime + " Segundos§e)");
						} else {
							Messages.sendBroadcast("§aO jogador §f" + gamers.getName() + "§a foi o primeiro a terminar a corrida §e(" + playerTime + " Segundos§e)");
							Messages.sendBroadcast("§aA corrida será finalizada em §f30 §asegundos!");
							setTime(30);
							chestplate = new ItemBuilder().setColor(Material.LEATHER_CHESTPLATE, Color.GREEN, "§aPeitoral do vencedor!");
						}
	...
	public void timer() {
		...
		if (playerTime % 10 == 0 && playerTime != 0 && end.size() == 0)
			gameManager.sendPlacementsMessage(manager.getUtils().getLocationFromConfig("starpvp.party.kangaroo.arrival"));
